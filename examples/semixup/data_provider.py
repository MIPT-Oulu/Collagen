...
class SemixupSampler(ItemLoader):
    def __init__(self, name: str, alpha: callable or float, model: nn.Module or None,
                 data_rearrange: callable or None = None,
    ...
    def sample(self, k=1):
        ...
        for i in range(k):
            ...
            imgs1_aug = []
            for b in range(imgs1_cpu.shape[0]):
                tmp_img = imgs1_cpu[b, :, :, :].astype(np.uint8)
                imgs1_aug.append(self.__augmentation({'image': tmp_img})['image'])
            imgs1_aug = torch.stack(imgs1_aug, dim=0)
